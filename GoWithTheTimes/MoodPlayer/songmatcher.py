...
def _score_songs(user_model, song_model_list):
        ...
        for song_model in song_model_list:
            ...
            song_scores_dict[song_model.song_id]=song_scores
        return song_scores_dict

def _pick_random_song(song_scores_dict):
        song_id = random.choice(song_scores_dict.keys(), weights=song_scores_dict.values(), k=1)
        return song_id
...
def generate_random_song_category(user_usermodel):
        ...
        pref3.rock=0
        pref3.jazz=1
        return random.choices([pref1,
                               pref2,
                               pref3, ],
                              weights=[user_pref.pop, user_pref.rock, user_pref.jazz], k=1)
